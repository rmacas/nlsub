...
def main(whdir, asddir, modeldir, outdir, mem):
    ...
    logger.info('Coloring and upsampling the noise')
    f_low = 10
    asd_win = 4
    firwin = scipy.signal.firwin(asd_win*fs+1, [f_low], pass_zero=False,
                                 window='hann', fs=fs)
    ...
    channel_clean = 'DCS-CALIB_STRAIN_NLSUB_C01_4096Hz_event.hdf5'
    tseries_clean.write(f'{outdir}/{channel_clean}', path='nlsub',
                        overwrite=True)
...
if __name__ == '__main__':
    ...
    main(args.whdir, args.asddir, args.modeldir, args.outdir,
         args.mem_reduction)
